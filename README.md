...
└── assets/
    ...
    │   └── skin-win.css        ← The alternate "SYSTEM" look.
    ├── js/
...
2. **The landing screen** — two buttons only: play the record, or join the list.
...
4. **The Inner Circle** — email only to start. Name and phone come after,
   once they've already said yes.
5. **The payoff** — a record to play immediately, and one tap to send it on.

Three places ask for the email, all running the same code: the section, the
slide-up bar (fires once they reach the videos), and a desktop exit prompt.
Once someone joins, **all three stop asking, permanently.**
...
| **Inner Circle** | Email + phone list signup |
